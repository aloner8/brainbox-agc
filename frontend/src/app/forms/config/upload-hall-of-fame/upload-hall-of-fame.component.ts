...
import { UploadHallOfFameDetailDialogComponent,LevelDetailDialogData } from './upload-hall-of-fame-dialog.component';
...
@Component({
    selector: 'upload-hall-of-fame',
    templateUrl: './upload-hall-of-fame.component.html',
    styleUrls: ['./upload-hall-of-fame.component.scss']
})
export class UploadHallOfFameComponent implements OnInit {
 ...
 */
    ...
    //console.log({order_by:this.order_by,page_size:this.page_size,page_select:this.page_select, order_by_desc:this.desc , filter_text:this.filter_text  })
        ...
        // this.api.Query("select id as running , description as fullname , minimum_point::text || ' - ' || maximum_point:text as level  , active as action  from public.level_masters",[]).subscribe(function(rows:any){
          this.api.Query("select id  , description as fullname , minimum_point::text || ' - ' || maximum_point::text  as level ,maximum_point , active as action  from public.level_masters order by id ",[]).subscribe(function(rows:any){
  ...
  //console.log(row)
      ...
      //console.log('level_masters:',rows)
             if(rows){
                 ...
                 const dialogRef = parent.DetailDialog.open(UploadHallOfFameDetailDialogComponent, {
                  data: _my_ideas,
                });
