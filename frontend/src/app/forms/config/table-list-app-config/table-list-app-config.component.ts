...
@Component({
    selector: 'table-list-app-config',
    templateUrl: './table-list-app-config.component.html',
    styleUrls: ['./table-list-app-config.component.scss']
})
export class TableListAppConfigComponent implements OnInit {
    ...
    displayedColumns: string[] = ['running', 'fullname', 'division' ,  'actions'];
  ...
  sort!: MatSort;
    ...
    //console.log({order_by:this.order_by,page_size:this.page_size,page_select:this.page_select, order_by_desc:this.desc , filter_text:this.filter_text  })
        ...
        // this.api.Query("select id as running , description as fullname , minimum_point::text || ' - ' || maximum_point:text as level  , active as action  from public.level_masters",[]).subscribe(function(rows:any){
          this.api.Query("select id as running , description as fullname ,value  as division ,  '' as action  from public.configs d  order by d.id ",[]).subscribe(function(rows:any){
...
OpenDetail(row:any){
  ...
  //console.log(row)
      if(row){
        ...
        parent.api.Query("select * from public.configs where id =$1 ",[row]).subscribe(function(rows:any){
      //console.log('level_masters:',rows)
             if(rows){
                 let _my_ideas = rows[0]
                 _my_ideas['IsAdmin'] = parent.IsAdmin                 
                 const dialogRef = parent.DetailDialog.open(AppConfigDialogComponent, {
                  data: _my_ideas,
                });
            
                dialogRef.afterClosed().subscribe(result => {
                  if(result){
                        parent.api.CallProcedure('public.update_configs',result).subscribe(function(updated){ 
                      //console.log(updated)                       
                         })
