...
@Component({
    selector: 'cus-approval-chart',
    templateUrl: './approval-chart.component.html',
    styleUrls: ['./approval-chart.component.scss'],
    providers: [
      
      ],
})
export class ApprovalChartComponent implements OnInit {
    ...
    const that = this;
            ...
            console.log('fliter_summary_month:',rows)
                   if(rows){
                       ...
                       that.approval_amount =rows['approval_amount'] 
                       that.reject_amount = rows['not_approval_amount'] 
                  ...
                  for (let index = 0; index < that.ApprovalSummary[that.veiw_type].length; index++) {
                    ...
                    if(index == that.ApprovalSummary[that.veiw_type].length - 1) {
                      ...
                      var chart = new CanvasJS.Chart(that.chartApproval,  that.columnChartOptions)
                      chart.render();
                      ...
                      if(that.approval_amount<9999){
                        if(that.approval_amount>999){
                          that.approval_amount_X =30
                        } else {
                          if(that.approval_amount>99){
                            that.approval_amount_X =40
                          } else {
                            if(that.approval_amount>9){
                              that.approval_amount_X =50
                            } else {
                              that.approval_amount_X =50
                            }
                          }
