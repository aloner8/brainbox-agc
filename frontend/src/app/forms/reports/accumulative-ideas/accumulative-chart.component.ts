...
@Component({
    selector: 'cus-accumulative-chart',
    templateUrl: './accumulative-chart.component.html',
    styleUrls: ['./accumulative-chart.component.scss'],
    providers: [
      
      ],
})
export class AccumulativeChartComponent implements OnInit {
 ...
 /*    readonly campaignOne = new FormGroup({
   ...
   ApprovalSummary:any ={M:[],Y:[]}
      ...
      this.BaseHref = this.API.getBaseHref();
       ...
       this.Parent.subscribe(function(d){
        //console.log(d)
            ...
            switch (d.eventname) {
                case 'ExportExcel':
                    if (d.activeView=='Accumulative Ideas'){
                      let _excelData:any =[]
    ...
    ExportJpg(){
      var chart = new CanvasJS.Chart("chartAccumulative",  this.columnChartOptions)
      chart.render();
      setTimeout(() => {
  ...
  //console.log(worksheet,param_worksheet)
    ...
    //console.log(event)
        that.columnChartOptions.data[0].dataPoints=[]
          ...
          this.API.CallProcedure('report_accumulative',{division:that.divitionfilter,select_year:that.selectMonth, select_month:that.selectMonth,select_year_end:that.selectMonthEnd,select_month_end:that.selectMonthEnd,status:that.statusfilter}).subscribe(function(rows:any){
        //console.log('report_accumulative:',rows)
               if(rows['summary_amount']){
            //console.log('hasrow',that.columnChartOptions.data[0].dataPoints)
                ...
                //console.log(that.columnChartOptions)
                   ...
                   var chart = new CanvasJS.Chart("chartAccumulative",  that.columnChartOptions)
                   setTimeout(() => {
                    chart.render(); 
                   }, 100);
