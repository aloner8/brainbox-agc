...
@Component({
    selector: 'cus-cost-saving-chart',
    templateUrl: './cost-saving-chart.component.html',
    styleUrls: ['./cost-saving-chart.component.scss'],
    providers: [
      
      ],
})
export class CostSavingChartComponent implements OnInit {
        ...
        data: [
          {
            ...
            legendText: "CO2 Reduction Plan (kg.)",             
            showInLegend: true,
            dataPoints: [
              { label: 'apple', y: 10 },
              { label: 'orange', y: 15 },
          ...
          } ,
          {
            ...
            indexLabelBorderThickness: 1 ,
            indexLabelFontSize: 12,
            legendText: "CO2 Reduction Actual (kg.)",               
            showInLegend: true,
            dataPoints: [
              { label: 'apple', y: 10 },
              { label: 'orange', y: 15 },
 ...
 /*    readonly campaignOne = new FormGroup({
   ...
   ApprovalSummary:any ={M:[],Y:[]}
      ...
      this.BaseHref = this.API.getBaseHref();
       ...
       this.Parent.subscribe(function(d){
        //console.log(d)
            ...
            switch (d.eventname) {
                case 'ExportExcel':
                    if (d.activeView=='Cost Saving by Division'){
                      let _excelData:any =[]
    ...
    ExportJpg(){
      var chart = new CanvasJS.Chart("chartCostSaving",  this.columnChartOptions)
      chart.render();
      setTimeout(() => {
  ...
  //console.log(worksheet,param_worksheet)
    ...
    //console.log(event)
        ...
        that.columnChartOptions.data[1].dataPoints=[]
          ...
          this.API.CallProcedure('report_cost_saving',{division:that.divitionfilter,select_year:that.selectMonth, select_month:that.selectMonth,select_year_end:that.selectMonthEnd,select_month_end:that.selectMonthEnd,status:that.statusfilter}).subscribe(function(rows:any){
        //console.log('report_cost_saving:',rows)
               if(rows){
                 ...
                 that.ApprovalSummary[that.veiw_type] = [].concat(rows) 
                  ...
                  // that.ApprovalSummary[that.veiw_type] =ApprovalSummary[that.veiw_type]
                   ...
                   for (let index = 0; index < that.ApprovalSummary[that.veiw_type].length; index++) {
                     ...
                     let _Onehardsaving = {label: element.column  , y: element.hardsaving }
                     that.columnChartOptions.data[0].dataPoints.push(_Onehardsaving)
                     let _Onesoftsaving = {label: element.column  , y: element.softsaving }
                     that.columnChartOptions.data[1].dataPoints.push(_Onesoftsaving)
                ...
                //console.log(that.columnChartOptions)
                   ...
                   var chart = new CanvasJS.Chart("chartCostSaving",  that.columnChartOptions)
                   setTimeout(() => {
                    chart.render(); 
                   }, 100);
