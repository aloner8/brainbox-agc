...
@Component({
    selector: 'cus-summary-division-chart',
    templateUrl: './summary-division-chart.component.html',
    styleUrls: ['./summary-division-chart.component.scss'],
    providers: [
      
      ],
})
export class SummaryDivisionChartComponent implements OnInit {
 ...
 /*    readonly campaignOne = new FormGroup({
   ...
   ApprovalSummary:any ={M:[],Y:[]}
      ...
      this.BaseHref = this.API.getBaseHref();
       ...
       this.Parent.subscribe(function(d){
        //console.log(d)
            ...
            switch (d.eventname) {
                case 'ExportExcel':
                    if (d.activeView=='Division Summary'){
                      let _excelData:any =[]
    ...
    ExportJpg(){
      var chart = new CanvasJS.Chart("chartSummaryDivision",  this.columnChartOptions)
      chart.render();
      setTimeout(() => {
  ...
  //console.log(worksheet,param_worksheet)
    ...
    //console.log(event)
        ...
        that.columnChartOptions.data[0].dataPoints=[]
          this.API.CallProcedure('report_summary_of_division',{division:that.divitionfilter,select_year:that.selectMonth, select_month:that.selectMonth,select_year_end:that.selectMonthEnd,select_month_end:that.selectMonthEnd,status:that.statusfilter}).subscribe(function(rows:any){
        //console.log('report_summary_of_division:',rows)
               if(rows){
                 ...
                 that.columnChartOptions['width'] = 1100;
                   ...
                   that.columnChartOptions['subtitles'] = [{From: that.dateformat.transform(that.selectMonth,'YYYY-MM-dd'),
                    ...
                    that.columnChartOptions['subtitles'] = [{ text: ' From: ' + that.dateformat.transform(that.selectMonth,'YYYY-MM-dd') + ' To: ' + that.dateformat.transform(that.selectMonthEnd,'YYYY-MM-dd') +'   Division : ' + _division_text   }]; 
                   

                   var chart = new CanvasJS.Chart("chartSummaryDivision",  that.columnChartOptions)
                   setTimeout(() => {
                    chart.render(); 
                   }, 100);
