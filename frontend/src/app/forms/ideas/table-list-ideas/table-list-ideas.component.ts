...
import { ExporttingDialog } from './exportting-dialog.component';
...
@Component({
    ...
    styleUrls: ['./table-list-ideas.component.scss']
})
export class TableListIdeasComponent implements OnInit {
    ...
    displayedColumns: string[] = ['running', 'title', 'idea_by', 'department_master_id' , 'current_status' , 'suggest_date' , 'pointideas'];
    listidieas:any = []
    statuslist:any = []
    categorylist:any = []
  ...
  rowOver =''
    ...
    constructor( private VariablesService: VariablesService 
       ...
       // console.log('_myideas:',users)
      //  this.myhome = this.VariablesService.getVariable('UserHome')
       
   
      //     this.statuslist = [].concat(this.myhome['status_list']) 
      //     this.coordinatorlist = this.myhome['coordinator_list']
      //     this.categorylist = this.myhome['category_list']
      //     this.approverlist = this.myhome['appover_list']
      //   //  const users = Array.from({length:  this.myhome['my_ideas'].length-1}, (_, k) => this.createNewUser(k + 1));
      //   //  console.log('myhome:',this.myhome )
    
      //     // Assign the data to the data source for the table to render
      //     this.dataSource = new MatTableDataSource(this.myhome['my_ideas']);
    ...
    //console.log('_ideas_filter:',_ideas_filter)
        if(_ideas_filter){
       ...
       this.StatusFilterChange(null)
        ...
        this.stateChanges.subscribe(function(v){
          //  var currentScreenSize=v['currentScreenSize'].toString()
       //console.log('stateChanges:',v)
           // that.setSize()
        })
   ...
   //console.log('paginatorChange:',event)
    ...
    //console.log(this.datefilter)
        let _input_parameter =  {order_by:this.order_by,page_size:this.page_size,page_select:this.page_select, order_by_desc:this.desc , filter_text:this.filter_text ,status_filter:this.statusfilter , approver_filter:this.approverfilter?.value ,coordinator_filter:this.coordinafilter?.value ,division_filter:this.divitionfilter?this.divitionfilter:0,date_filter:this.datefilter , category_filter:this.categoryfilter    }
   ...
   //console.log('fliter_ideas:',rows)
      ...
      FilterOnePage(pagenumber:any){
        ...
        let _input_parameter =  {order_by:this.order_by,page_size:this.page_size,page_select:pagenumber, order_by_desc:this.desc , filter_text:this.filter_text ,status_filter:this.statusfilter , approver_filter:this.approverfilter?.value ,coordina_filter:this.coordinafilter?.value ,division_filter:this.divitionfilter?this.divitionfilter:0,date_filter:this.datefilter , category_filter:this.categoryfilter    }
      ...
      //console.log('fliter_ideas:',rows)
       ...
       ExportOnePage(pagenumber:any){
        ...
        let _input_parameter =  {order_by:this.order_by,page_size:this.page_size,page_select:pagenumber, order_by_desc:this.desc , filter_text:this.filter_text ,status_filter:this.statusfilter , approver_filter:this.approverfilter?.value ,coordina_filter:this.coordinafilter?.value ,division_filter:this.divitionfilter?this.divitionfilter:0,date_filter:this.datefilter , category_filter:this.categoryfilter    }
      console.log('_input_parameter:',_input_parameter)
          this.api.CallProcedure('export_ideas_list',_input_parameter).subscribe(function(rows:any){
  ...
  ExportExcel(sheetname:string,ExcelData:any,param:any){
    ...
    createNewUser(id:number): MyIdeasData {
          ...
          const _myideas = this.listidieas
